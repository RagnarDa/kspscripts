...
function GLIDE {
	...
	UNTIL (ALT:RADAR - 5 < HOVERSLAMALT)// - (SHIP:VERTICALSPEED*1.1))
	{
		// SIMPLE GLIDE ALGO, JUST AIM AT TARGET.
		// Surprisingly effective.
		SET TARGETDIST TO KSC:DISTANCE. // MAYBE THIS IS GROUND DIST??????
		SET CURALT TO ALTITUDE.
		
		PRINT "ALTITUDE " + ALTITUDE.
		PRINT "DISTANCE " + TARGETDIST.
		SET GLIDEPATH TO ARCTAN(ALTITUDE-500/TARGETDIST). // HACK: Aim 500m above.
		SET NORTHERROR TO (KSC:LAT-SHIP:GEOPOSITION:LAT).
		PRINT "GLIDEPATH " + (GLIDEPATH).
		PRINT "ALPHA " + GETALPHADEG(CALCHORV,SHIP:VERTICALSPEED,SHIP:FACING:PITCH).
		PRINT "NORTHERROR " + NORTHERROR * 2.
		LOCK STEERING TO HEADING(-90+(NORTHERROR*2),(GLIDEPATH)).
		WAIT 0.1.
		LOCAL TIMETOCRASH TO ALTITUDE/(-SHIP:VERTICALSPEED).
		SET HOVERSLAMALT TO GETHOVERSLAMALTITUDE((SHIP:MASS + SHIP:DRYMASS)/2,SHIP:MAXTHRUST,GETGATALT(ALTITUDE),SHIP:VERTICALSPEED).
		PRINT "HOVERSLAM ALT: " + HOVERSLAMALT.
		PRINT "STOP TIME: " + GETSTOPTIME((SHIP:MASS + SHIP:DRYMASS)/2,SHIP:MAXTHRUST,GETGATALT(ALTITUDE),-SHIP:VERTICALSPEED).
	}
	LOCK STEERING TO UP.
	LOCK THOTTLE TO 1.
	WAIT GETSTOPTIME((SHIP:MASS + SHIP:DRYMASS)/2,SHIP:MAXTHRUST,GETGATALT(ALTITUDE),-SHIP:VERTICALSPEED).
	UNTIL(FALSE)
	{
...
function GETHOVERSLAMALTITUDE {
	...
	RETURN GETDISPLACEMENT(-VERTICALVELOCITY,GETSTOPTIME(SHIPWEIGHT, SHIPTHRUST, GRAVITY, -VERTICALVELOCITY), ((SHIPTHRUST/SHIPWEIGHT)-GRAVITY)).
}
