...
function ASCEONDTOORBIT {
	...
	UNTIL (APOAPSIS > TARGETALT * 0.999 AND PERIAPSIS > TARGETALT * 0.999)
	{
		...
		// Combine the two.
		LOCK THROTTLE TO ASCENTBURN+CIRCBURN.//+GETALPHADEG(CALCHORV,SHIP:VERTICALSPEED,SHIP:FACING:PITCH).
...
function ALTASCEND {
	UNTIL (PERIAPSIS > TARGETALT)
	{
		...
		LOCK STEERING TO HEADING(90, 90-(90*(APOAPSIS/TARGETALT))). // 90 - (90*(50/100)) = 45,  90 - (90*(100/100)) = 0
		wait 0.1.
	}
	LOCK THROTTLE TO 0.
}

// And a little more complicated. Make time to apopapsis stable.
function ALTASCEND2 {
	SET LASTTHROTTLE TO 1.0.
	SET TIMETOAPO TO (APOAPSIS-ALTITUDE)/-SHIP:VERTICALSPEED.
	SET LASTTIMETOAPO TO (APOAPSIS-ALTITUDE)/-SHIP:VERTICALSPEED.
	SET NEWTHROTTLE TO 1.0.	
	UNTIL (PERIAPSIS > TARGETALT)
	{
		SET TIMETOAPO TO (APOAPSIS-ALTITUDE)/-SHIP:VERTICALSPEED.
		
		IF (LASTTIMETOAPO > TIMETOAPO)
		{
			SET NEWTHROTTLE TO LASTTHROTTLE + 0.05.
		} ELSE {
			SET NEWTHROTTLE TO LASTTHROTTLE - 0.05.
		}
		SET LASTTHROTTLE TO NEWTHROTTLE.
		SET LASTTIMETOAPO TO TIMETOAPO.
		LOCK STEERING TO HEADING(90, 90-(90*(APOAPSIS/TARGETALT))). // 90 - (90*(50/100)) = 45,  90 - (90*(100/100)) = 0
		wait 0.5.
	}
	LOCK THROTTLE TO 0.
...
// Yet another profile (https://www.reddit.com/r/Kos/comments/3ijayc/recommended_ascent_curves/)
function ALTASCEND3 {
	TransferFor().
	SET TIPOVER TO 89.
	UNTIL (TIPOVER < 60)
	{
		LOCK STEERING TO HEADING(90,TIPOVER). 
		// Just a little gentler turn-over
		WAIT 0.5.
		SET TIPOVER TO TIPOVER - 1.
	}
	UNTIL (PERIAPSIS > TARGETALT)
	{
		LOCAL altS IS 100.
		...
		LOCAL endPitch IS 10. //this value is in flux
		LOCAL expo IS 0.7. //Controls shape of curve; pre 1.0 this was 0.5; still tweaking for new FAR.
		LOCAL pitch IS endPitch + (90-endPitch)*(1-ALTPER^(expo-(ALTPER/(1/expo)))).
		print "pitch: " + pitch.
		LOCK STEERING TO HEADING(90, pitch).
		LOCK THROTTLE TO 1.5 / GETTWR(ALTITUDE). // Apparantly ascend at 1.5 twr is most efficient?
		wait 0.5.
	}
	LOCK THROTTLE TO 0.
}
...
function GETLONGGROUNDDIST {
	parameter POS.
	// Calculating geographical distance.
	SET POSDISTGEO TO POS:LNG-SHIP:GEOPOSITION:LNG.
	IF (POSDISTGEO < 0)
	{
		SET POSDISTGEO TO POSDISTGEO + 360.
	}
...
function DEORBIT {
	...
	UNTIL (IMPACTDIST < Kerbin:RADIUS * CONSTANT:PI AND IMPACTDIST > 0)
	{
		...
		SET IMPACTDISTE TO GETEULERBALLX(DEORBITALT, 0, DEORBITV, DEORBITVV, Cd)[0].
		PRINT "IMPACTDIST: " + IMPACTDIST AT (0,10).
		PRINT "DEORBITV: " + DEORBITV AT (0,11).
		PRINT "EULER: " + IMPACTDISTE AT (24,10).
		SET IMPACTDIST TO IMPACTDISTE.
	}
	...
	set kuniverse:timewarp:warp to 0.
	SET LASTDIFF TO GETLONGGROUNDDIST(KSC)-IMPACTDIST.
	SET DEORBITV TO GETCURVEDDIST(DEORBITV, DEORBITALT).
	SET DIFF TO GETLONGGROUNDDIST(KSC)-IMPACTDIST.
	...
	UNTIL (0 > (TIMETODEORBITBURN - (BURNTIME*2.0))) // HACK!
	{
		...
		PRINT "DIST: " + GETLONGGROUNDDIST(KSC) AT (0,13).
	...
	LOG "DEORBITED AT ALT "+ altitude + " DIST " + GETLONGGROUNDDIST(KSC) + " WITH IMPACTX " + IMPACTDIST TO "mylog".
	LOCK THROTTLE TO 0.
	UNTIL (ALTITUDE < 50000) // Still outside atmosphere...
	{
		
		// Since the trajectory somehow underestimates the distance
		// we need to make additional burns on our way down.
		...
		SET OVERSHOOT TO DECISIONTRAJ[0]-GETLONGGROUNDDIST(KSC).
		PRINT "WILL OVERSHOOT: " + OVERSHOOT.
		LOG "WILL OVERSHOOT: " + OVERSHOOT TO "mylog".
		LOCAL NEWTARGETV TO SHIP:AIRSPEED * (GETLONGGROUNDDIST(KSC) / DECISIONTRAJ[0]).
		PRINT "NEW TARGET V: " + NEWTARGETV.
		UNTIL (NEWTARGETV < SHIP:AIRSPEED OR OVERSHOOT < 0)
		{
			// VEEERY HACKISH.
			LOCK STEERING TO RETROGRADE.
			LOCK THROTTLE TO 1.0.
	...
	LOG "FINISHED DEORBIT AT ALT "+ altitude + " DIST " + GETLONGGROUNDDIST(KSC) + " WITH IMPACTX " + IMPACTDIST TO "mylog".
...
function GLIDE {
	PRINT "GLIDING...".
	TransferAft().
	SET USEBAMBAMGLIDING TO FALSE.
	LOCAL EXPECTEDHOVERSLAMALT TO GETEXPECTEDHOVERSLAMALTITUDE(SHIP:MASS,SHIP:MAXTHRUST,GETGATALT((0+ALTITUDE)/2),SHIP:VERTICALSPEED,ALTITUDE).
	
	if (USEBAMBAMGLIDING = FALSE)
	{
	...
	LOCAL EXPECTEDHOVERSLAMALT TO GETEXPECTEDHOVERSLAMALTITUDE(SHIP:MASS,SHIP:MAXTHRUST,GETGATALT((0+ALTITUDE)/2),SHIP:VERTICALSPEED,ALTITUDE).
	LOCAL REORIENTATIONTIME TO 5. // HACK
	LOG "BEGUN GLIDING AT ALT "+ altitude + " DIST " + GETLONGGROUNDDIST(KSC) TO "mylog".
	SET TARGETDIST TO KSC:DISTANCE.
	SET CURALT TO ALT:RADAR.
	LOG "EXPECTED HOVERSLAM ALT " + EXPECTEDHOVERSLAMALT TO "mylog".
	LOG "ALTITUDE " + CURALT TO "mylog".
	LOG "DISTANCE " + TARGETDIST TO "mylog".
	IF (CURALT > TARGETDIST)
	{
		SET CURALT TO TARGETDIST.
	}.
	SET GLIDEPATH TO ARCSIN((CURALT)/TARGETDIST).
	SET NORTHERROR TO (KSC:LAT-SHIP:GEOPOSITION:LAT).
	LOG "GLIDEPATH " + (GLIDEPATH) TO "mylog".
	LOCAL BACKWARDSALPHA TO 90-GETALPHADEG(CALCHORV,SHIP:VERTICALSPEED,SHIP:FACING:PITCH).
	LOG "BACKWARDSALPHA " + BACKWARDSALPHA TO "mylog".
	LOG "NORTHERROR " + NORTHERROR * 2 TO "mylog".
	LOG "GLIDE ERROR: " + (SHIP:FACING:PITCH - (GLIDEPATH-BACKWARDSALPHA)) TO "mylog".
	UNTIL ((ALT:RADAR - 5 - (1*(-SHIP:VERTICALSPEED))) < HOVERSLAMALT * 1)// - (SHIP:VERTICALSPEED*1.1))
	{
		// SIMPLE GLIDE ALGO, JUST AIM AT TARGET.
		
		SET TARGETDIST TO KSC:DISTANCE.
		
		SET CURALT TO ALT:RADAR * 2.0.
		PRINT "EXPECTED HOVERSLAM ALT " + EXPECTEDHOVERSLAMALT.
		...
		SET GLIDEPATH TO ARCSIN((CURALT)/TARGETDIST).
		//SET ALTGLIDEPATH TO ARCSIN((CURALT-EXPECTEDHOVERSLAMALT)/TARGETDIST).

		SET NORTHERROR TO (KSC:LAT-SHIP:GEOPOSITION:LAT).
		PRINT "GLIDEPATH " + (GLIDEPATH).
		//PRINT "ALT GLIDEPATH " + ALTGLIDEPATH.
		//SET GLIDEPATH TO ALTGLIDEPATH.
		LOCAL BACKWARDSALPHA TO 90-GETALPHADEG(CALCHORV,SHIP:VERTICALSPEED,SHIP:FACING:PITCH).
		LOCAL VELOCITYVECTOR TO ARCTAN(SHIP:VERTICALSPEED/CALCHORV).
		LOCAL GLIDEERROR TO GLIDEPATH - VELOCITYVECTOR. // Overshoot = positive.
		PRINT "GLIDEERROR " + GLIDEERROR.
		PRINT "BACKWARDSALPHA " + BACKWARDSALPHA.
		PRINT "NORTHERROR " + NORTHERROR * 2.
		SET STEER TO -90.
		IF (KSC:LNG < longitude)
		{
			// Passed over KSC.
			SET STEER TO 90.
		}
		LOCK STEERING TO HEADING(KSC:HEADING - 180,(GLIDEPATH + (GLIDEERROR*2))).
		//LOCK STEERING TO UP. // HACK HACK HACK!
		//LOCK STEERING TO HEADING(-90,0). //HACK
		WAIT 0.1.
		LOCAL TIMETOCRASH TO ALTITUDE/(-SHIP:VERTICALSPEED).
		LOCAL MASSREQUIRED TO CALCULATEMASSREQUIRED(GETGATALT(ALTITUDE),260,SHIP:MASS,ABS(SHIP:VERTICALSPEED)).

		SET HOVERSLAMALT TO GETHOVERSLAMALTITUDE(SHIP:MASS - MASSREQUIRED,SHIP:MAXTHRUST,GETGATALT((0+ALTITUDE)/2),SHIP:VERTICALSPEED).
		...
		LOCAL TIMETOHOVER TO ALTITUDE-HOVERSLAMALT/(-SHIP:VERTICALSPEED).
		IF (TIMETOHOVER < REORIENTATIONTIME)
		{
			// Reorient just before hover slamming
			LOCK STEERING TO UP.
		}
	}
	LOG "FINISHED GLIDING AT ALT "+ altitude + " DIST " + GETLONGGROUNDDIST(KSC) TO "mylog".
	SET TARGETDIST TO KSC:DISTANCE.
	SET CURALT TO ALT:RADAR.
	LOG "FINAL HOVERSLAM ALT " + HOVERSLAMALT TO "mylog".
	LOG "ALTITUDE " + CURALT TO "mylog".
	LOG "DISTANCE " + TARGETDIST TO "mylog".
	IF (CURALT > TARGETDIST)
	{
	...
	UNTIL (SHIP:VERTICALSPEED > -5.0)
	{ 
		wait 0.1.
	}.
	LOCK THROTTLE TO 0.
	LOG "FINISHED HOVERSLAM AT ALT "+ altitude + " DIST " + GETGROUNDDIST(KSC) TO "mylog".
...
//	TransferAft().
	...
	IF (USEBAMBAMGLIDING = TRUE)
	{
	UNTIL (ALTITUDE < EXPECTEDHOVERSLAMALT)
	{
		...
		IF (SHIP:VERTICALSPEED < 0)
		{
			SET TARGETDIST TO KSC:DISTANCE.//GETGROUNDDIST(KSC).
			...
			LOG "IMPACTERROR: " + IMPACTERROR TO "mylog".
			LOG "TARGETDIST: " + TARGETDIST TO "mylog".
			LOG "TRAJDIST: " + TRAJDIST TO "mylog".
			LOG "ESTDIFF: " + ESTDIFF TO "mylog".
			IF (IMPACTERROR > 100)
			{
...
// Returns the altitude which is expected to start full thrust to not hit ground.
function GETEXPECTEDHOVERSLAMALTITUDE {
	parameter SHIPWEIGHT.
	parameter SHIPTHRUST.
	parameter GRAVITY.
	parameter VERTICALVELOCITY.
	parameter CURRENTALTITUDE.
	// HACKY QUICK INEXACT CALCULATION TODO
	LOCAL TIMETOIMPACT TO CURRENTALTITUDE / -VERTICALVELOCITY.
	LOCAL VATIMPACT TO -VERTICALVELOCITY + (GRAVITY*TIMETOIMPACT*0.5).
	LOCAL TIMETOIMPACT2 TO CURRENTALTITUDE / (VATIMPACT).
	LOCAL VATIMPACT2 TO -VERTICALVELOCITY + (GRAVITY * TIMETOIMPACT2).
	PRINT "TIMETOIMPACT " + TIMETOIMPACT.
	PRINT "VATIMPACT " + VATIMPACT.
	PRINT "TIMETOIMPACT2 " + TIMETOIMPACT2.
	PRINT "VATIMPACT2 " + VATIMPACT2.
	//RETURN GETDISPLACEMENT(-VERTICALVELOCITY,GETSTOPTIME(SHIPWEIGHT, SHIPTHRUST, GRAVITY, -VERTICALVELOCITY), ((SHIPTHRUST/SHIPWEIGHT))).
	RETURN 0.5 * VATIMPACT2 * GETSTOPTIME(SHIPWEIGHT, SHIPTHRUST, GRAVITY, VATIMPACT2).
}
Assert((GETEXPECTEDHOVERSLAMALTITUDE(50,1000,10,-20,10000)>GETEXPECTEDHOVERSLAMALTITUDE(50,2000,10,-20,5000)), "Expected hover slam altitude 1").
