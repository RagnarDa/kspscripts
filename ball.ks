...
function TRAJECTORYSTEP {
	...
	SET NEWHORV TO CURRENTHORV - ((AERODYNAMICFORCEHOR / SMASS)*DT).
	SET NEWVERV TO 0.
	IF (CURRENTVERV > 0)
	{
		SET NEWVERV TO CURRENTVERV - ((AERODYNAMICFORCEVER / SMASS) * DT).
	} ELSE {
		// I guess if falling the vertical drag is applied upwards.
		SET NEWVERV TO CURRENTVERV + ((AERODYNAMICFORCEVER / SMASS) * DT).
	}
	// Apply G.
	SET NEWVERV TO NEWVERV - (GETGATALT(CURRENTALT) * DT).
	...
	SET DISTANCEX GETCURVEDDIST(CURRENTHORV, CURRENTALT).
...
function RK4Step {
	...
	SET x1 TO CURRENTHORV + (DT * f0[1] / 2.0). 
	...
	SET x2 TO CURRENTHORV + (DT * f1[1] / 2.0).
	...
	SET x3 TO CURRENTHORV + (DT * f2[1]).
  	SET f3 TO TRAJECTORYSTEP(a3, v3, u3, DT).//f3 = f ( t3, u3 );

	//
	//  Combine to estimate the solution at time T0 + DT.
	//
	SET a TO CURRENTVERV + DT * (f0[0] + 2.0 * f1[0] + 2.0 * f2[0] + f3[0]) / 6.0. //u = u0 + dt * ( f0 + 2.0 * f1 + 2.0 * f2 + f3 ) / 6.0;
  	SET u TO CURRENTVERV + DT * (f0[3] + 2.0 * f1[3] + 2.0 * f2[3] + f3[3]) / 6.0. //u = u0 + dt * ( f0 + 2.0 * f1 + 2.0 * f2 + f3 ) / 6.0;
	SET v TO CURRENTVERV + DT * (f0[2] + 2.0 * f1[2] + 2.0 * f2[2] + f3[2]) / 6.0. //u = u0 + dt * ( f0 + 2.0 * f1 + 2.0 * f2 + f3 ) / 6.0;
	SET x TO CURRENTVERV + DT * (f0[1] + 2.0 * f1[1] + 2.0 * f2[1] + f3[1]) / 6.0. //u = u0 + dt * ( f0 + 2.0 * f1 + 2.0 * f2 + f3 ) / 6.0;
...
function DEORBIT {
	...
	{
		SET DEORBITV TO DEORBITV * 0.99.
		SET IMPACTDIST TO GETEULERBALLX(DEORBITALT, 0, DEORBITV,0, Cd).
		PRINT "IMPACTDIST: " + IMPACTDIST AT (0,10).
		PRINT "DEORBITV: " + DEORBITV AT (0,11).
	}
	SET LASTDIFF TO GETGROUNDDIST(KSC)-IMPACTDIST.
	UNTIL (GETGROUNDDIST(KSC) < IMPACTDIST)
	{
	...
	UNTIL (DEORBITV > CALCHORV)
	{
		PRINT "REMAINING DEORBIT V: " + (CALCHORV - DEORBITV) AT (0,13).
...
PRINT GETEULERBALLX(90000, 0, 2800,0, Cd).
