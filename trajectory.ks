...
PRINT "GROUNDE: " + (sdLonV * (bodyRadius+sdAlt)*sdDegToRad).
PRINT "GROUNDN: " + (sdLatV * (bodyRadius+sdAlt)*sdDegToRad).
PRINT "SURVVECT X: " + ship:velocity:surface:x + " Y: " + ship:velocity:surface:y + " Z: " + ship:velocity:surface:z.
PRINT "SURFVEL: " + SQRT((ship:velocity:surface:x*ship:velocity:surface:x)+(ship:velocity:surface:z*ship:velocity:surface:z)).
PRINT "SURFVEL: " + SQRT((ship:velocity:surface:x*ship:velocity:surface:x)+(ship:velocity:surface:y*ship:velocity:surface:y)).
SET GROUNDDIR TO ARCTAN2(sdLatV,sdLonV).
PRINT "GROUNDDIR: " + (GROUNDDIR).
PRINT "SURFDIR: " + ship:velocity:surface:direction.
PRINT "SURF/NORTH: " + VANG(NORTH:VECTOR, (-1)*ship:velocity:surface).
...
//LOCK STEERING TO retroSurf - V(0,90,0).
...
SET latspeed TO (ship:velocity:surface:z*180)/constant():pi/300000.
SET lonspeed TO (ship:velocity:surface:y*180)/constant():pi/300000.
SET spot TO LATLNG(SHIP:GEOPOSITION:LAT+(latspeed*distratio),SHIP:GEOPOSITION:LNG+(lonspeed*distratio)).
PRINT "SPOT: " + spot.
...
until (FALSE)
{
	...
	SET VD TO VECDRAWARGS(
               V(0,0,0),//spot:ALTITUDEPOSITION(spot:TERRAINHEIGHT+1000),
              ship:POSITION - spot:ALTITUDEPOSITION(spot:TERRAINHEIGHT),
              red, "IMPACT SPOT", 1, true).
	SET VV TO VECDRAWARGS(
              V(0,0,0),
              velocity:surface,
              blue, "SURFACE VECTOR", 1, true).
